...
def handle_command(command, channel):
    ...
    if command.startswith('bitbucket license'):
         response = "Count of Bitbucket licenses: {}".format(user_count_license.get_bitbucket_license())
    elif command.startswith('jira license'):
         response = "Count of Jira Software licenses: {}".format(user_count_license.get_jira_license())
    elif command.startswith('confluence license'):
         response = "Count of Confluence licenses: {}".format(user_count_license.get_confluence_license())
    elif command.startswith('license'):
         response = "Count of Confluence licenses: {}\n Count of Jira licenses: {}\n Count of Bitbucket licenses: {}" \
         .format(user_count_license.get_confluence_license(),user_count_license.get_jira_license(),user_count_license.get_bitbucket_license())
    elif command.startswith('expiration date'):
         response = "Atlassian Applications Licenses will expire in: {} days".format(expiration_date.get_bitbucket_expiration_date())
